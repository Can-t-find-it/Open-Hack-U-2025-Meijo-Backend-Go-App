...
func (h *PenaltyHandler) CheckPenalty(c *gin.Context) {
	// ★ 修正点: c.Param("userID") から c.Get("userID") に変更
	userIDFromContext, exists := c.Get("userID") 
    fmt.Printf("%v\n", userIDFromContext)
	if !exists {
		// ミドルウェアでuserIDがセットされていない（認証されていない）場合は401を返す
		c.JSON(http.StatusUnauthorized, gin.H{"error": "認証ユーザー情報が見つかりません"})
		c.Abort()
		return
	}
	// string型に型アサーション
	postUserID := userIDFromContext.(string) // ← ログインユーザー

	var friends []models.Friend
	result := database.DB.Where("user_id = ?", postUserID).Find(&friends)
	...
	for _, fid := range friendIDs {
		...
		var u models.User
		userResult := database.DB.Where("id = ?", fid).First(&u)
...
// --- APNs 通知 ---
func (h *PenaltyHandler) sendPenaltyNotification(userID string, name string) {
